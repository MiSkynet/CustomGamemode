...
public class PayCommand implements BasicCommand {
    ...
    public void execute(CommandSourceStack commandSourceStack, String[] args) {
        ...
        if (target == null) {
            player.sendMessage(Utils.component("&cPlayer " + args[1] + " is not a valid player"));
            return;
        }

        if (target.getName().equalsIgnoreCase(player.getName())) {
            player.sendMessage(Utils.component("&cYou cannot send yourself money!"));
            return;
        }
        ...
        try {
            ...
            player.sendMessage(Utils.component("&cPlease enter a valid amount!"));
            return;
        }

        if (amount < 0) {
            player.sendMessage(Utils.component("&cYou have to at least send 1" + ecoManager.getEcoSymbol()));
            return;
        }

        if (amount > ecoManager.getBalance(player)) {
            player.sendMessage(Utils.component("&cYou cannot send " + amount + ecoManager.getEcoSymbol() +
        ...
        player.sendMessage(Utils.component("&7You payed " + target.getName() + " " + amount + ecoManager.getEcoSymbol()));
        target.sendMessage(Utils.component("&7" + player.getName() + " payed you " + amount + ecoManager.getEcoSymbol()));
